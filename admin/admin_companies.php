...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'delete') {
        // Delete company
        $delete_sql = "DELETE FROM users WHERE id = ? AND user_type = 'company'";
...
// Build query
$query = "SELECT u.*, cp.company_name, cp.location 
          FROM users u 
          LEFT JOIN company_profile cp ON u.id = cp.user_id 
          WHERE u.user_type = 'company'";
$countQuery = "SELECT COUNT(*) as total FROM users WHERE user_type = 'company'";
...
if (!empty($search)) {
    $whereClause[] = "(u.name LIKE ? OR u.email LIKE ? OR cp.company_name LIKE ?)";
    $searchParam = "%$search%";
    $params[] = $searchParam;
    $params[] = $searchParam;
    $params[] = $searchParam;
    ...
    <div class="wrapper">
        <div class="sidebar">
            ...
            <div class="sidebar-menu">
                ...
                <a href="admin_companies.php" class="menu-item active">
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($company = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($company['company_name'] ? $company['company_name'] : $company['name']); ?></td>
                                <td><?php echo htmlspecialchars($company['email']); ?></td>
                                <td><?php echo htmlspecialchars($company['location'] ? $company['location'] : 'Not specified'); ?></td>
                                <td><?php echo date('M d, Y', strtotime($company['created_at'])); ?></td>
                                <td class="action-links">
                                    <a href="admin_view_company.php?id=<?php echo $company['id']; ?>">
                                        <i class="fas fa-eye"></i> View
                                    </a>
                                    <a href="admin_edit_company.php?id=<?php echo $company['id']; ?>">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <a href="admin_companies.php?action=delete&id=<?php echo $company['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this company?');">
